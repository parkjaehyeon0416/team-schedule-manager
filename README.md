...
- 웹·모바일 모두 작성/승인 가능합니다. PDF 다운로드는 현재 웹 대시보드에서만 지원합니다.
...
- 웹·모바일 모두 조회/생성/가입을 지원합니다(수정·삭제는 웹 전용).

### 📄 자동 보고서 (PDF) + 공유 링크
- 현장이 연결된 일정에 대해 시공 완료 보고서를 PDF로 자동 생성합니다.
- 고객명 · 연락처 · 인사말을 입력하면 현장 정보 + 공정 정보 + 시공 전/중/후 카테고리별 사진 + 시공 전·후 비교 이미지가 포함된 PDF가 만들어집니다.
- 웹 대시보드의 일정 상세(캘린더에서 일정 클릭)에서, 모바일은 일정 상세 화면의 "자동 보고서 관리"에서 생성 · 공유 · 삭제할 수 있습니다.
- 고객은 로그인 없이 공유 링크(`/api/report/{share_token}`) 하나로 PDF를 브라우저에서 바로 열람할 수 있으며, 열람 횟수 · 마지막 열람 시각이 자동으로 기록되어 웹·모바일 양쪽에서 확인할 수 있습니다.
...
- 조회수(누적/이번 달)를 웹·모바일 양쪽에서 확인할 수 있습니다.

### 🧾 수입·경비 정리 (세무사용)
- 연도를 선택하면 월별 근무일수 · 공수 · 수입 · 경비 · 예상 원천징수액 · 실수령액을 한눈에 볼 수 있습니다.
- 세무사에게 바로 전달할 수 있는 PDF로 다운로드할 수 있습니다(웹 대시보드 전용, 모바일은 화면 조회만 지원).
- 예상 원천징수액은 일용근로소득 기준 간이 추정치입니다(일당에서 150,000원 공제 후 2.7%). 실제 신고는 세무 전문가와 상담이 필요합니다.
...
| 세무 | GET | `/api/tax-summary` | 연간 수입·경비 정리 조회 (`?year=`) | member+ |
| 세무 | GET | `/api/tax-summary/pdf` | 수입·경비 정리 PDF 다운로드 (`?year=`) | member+ |
